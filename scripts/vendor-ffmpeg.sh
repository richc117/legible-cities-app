...
# fingerprints the pins record (signed(), in scripts/vendor-signature.sh),
# copies of this script, that helper, the pins and the vendor workflow, and
# BUILD.txt with every configure line and the
...
cd "$root"
# signed and signature_status, shared with the loom-source job.
# shellcheck source=scripts/vendor-signature.sh
. "$root/scripts/vendor-signature.sh"
...
"
  ...
  cp scripts/vendor-ffmpeg.sh scripts/vendor-signature.sh .github/workflows/vendor.yml "$outdir/build/"
  cp "$pins" "$outdir/build/pins.json"
  commit=${GITHUB_SHA:-$(git rev-parse HEAD 2>/dev/null || echo unknown)}
  changed=$(git status --porcelain -- scripts/vendor-ffmpeg.sh scripts/vendor-signature.sh .github/workflows/vendor.yml vendor/pins.json 2>/dev/null || true)
  ...
  fi
  {
    ...
    echo "Copies of both files, of scripts/vendor-signature.sh, which the script"
    echo "sources, and of vendor/pins.json, as they were then are in build/."
    echo
