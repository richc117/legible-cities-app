...
| Progress and steps | the line vocabulary (section 10): one line per run with a round station per stage, a finished stage filled, the running one ringed in `--accent` and the line reaching it, a failed one in `--error`; a station is never told apart by colour alone, so the sentence beside the line names the stage and its state; where the line turns it turns on a rounded quarter-circle, never a 45-degree join; the sentence from the engine beside it at 13px; cancel is a text button to the right; the controls a run offers change as it moves - Lay out gives way to Cancel, Cancel to Lay out again, Export to Cancel to Reveal - and focus on the one that went is handed to the one that came, without scrolling, only when the control that last took focus there can no longer hold it and focus has nowhere else to be; a press on prose forgets it (A6-07). On the layout run its stations carry **the app's words, not the engine's** (A5.5-10): `parse`, `collapse`, `order`, `octilinear`, `trips`, `draw`, `animate`, `write`, one per engine stage and in the engine's order, from the one exported table in `src/renderer/src/stages.ts` that a totality test holds - so a stage the engine grows fails a check rather than vanishing from the screen. The sentences spoken beside the line stay the engine's own, and so do the jobs inspector's copied log and the geographic view's two buttons, because those are what a person quotes at the engine; the inspector's own stations take the app's words, since two vocabularies for one run on one screen is what a person has to stop and resolve. The line is drawn at rest too, before anything is pressed, every station pending: "The layout run's 8 stages, none started." on a project never laid out, "The layout run's 8 stages." on one already laid out, because the second clause is about the run and would otherwise be read as being about the project |
...
| Panels and settings | a 15px heading - an `h3` where the panel sits inside a cell of the notebook, a level below the cell's own row, and the panel's `aria-labelledby` target either way, so the name a test or a walkthrough reaches the region by does not move with the level (issue 197) - rows of label at 13px and control at `--control-height` 32px, `--space-4-3` between rows, `--space-4-6` between groups, one column |
...
| The transport (A5.5-16) | cell 03's second section, which drives the engine's page and never the project. It names itself with an `h3`, where the day beside it is named by the cell's own heading and an `aria-label`: a panel takes one form or the other and never an `h2`, and which one is a judgement about the panel. The cell is called Frame and service day, so a heading repeating that under it would add a level for nothing and the day's focus handback lands on the cell's heading for the same reason; the transport is about the map on screen rather than what the project keeps, so nothing above names it. Then one sentence saying it changes nothing kept, the platform's native `<input type="range">` for the clock - stepped a minute, `accent-color: var(--accent)`, `aria-valuetext` the clock the page wrote, the same clock beside it in `--numerals` and never in a live region - and a row of actions holding Play day/Pause and the speed as the kit's select. Nothing here disables while a run, an export or cell 06's preview holds the page: the press is refused with one sentence in `--error`, because a control that disables itself under a person's hands takes the focus with it, and a scrub kept for later would move a map that had since been redrawn. It draws nothing until the page has answered `bounds()`, and never takes itself off the screen afterwards. It adds no colour pair: `--text`, `--text-muted`, `--accent` and `--error` on `--surface` are all already checked |
...
| Export tab (A5-01) | one sentence saying the map shows the export's frame while cell 06 is open (its own tab until A5.5-08), then panel rows in one column: the preset as a native select with an `<optgroup>` per platform, each option the engine's name, its size and what it makes; the storyboard as a native select for a video or GIF preset only, each option its views and length, the preset's own marked; the engine's refusal of the choice in `--error` at 13px directly under those two, where the choice was made, and Export disabled until the choice changes; the view as a native select, for a still only, since a storyboard's first beat names its own view and clock; the quality as a native select, and for a JPEG still (the engine's table says which) a disabled select at standard with one message line saying the engine makes a JPEG still at standard quality only; the frame's three switches and the lines to keep as native checkboxes in `<fieldset>`s whose `<legend>` reads as a field's label, `accent-color` from `--accent`, each row at least `--target-min`; the start time (a still's only) and the filename tag as the kit's text field with their rule beneath as the field's message, written when committed (Enter or leaving the field), never on each keystroke, and a sentence in `--error` there when the engine's pattern refuses it; then the export's own progress line, cancel and Reveal. Every choice is written to the project the moment it is made. While an export runs the choices are disabled, with a `role="status"` line saying why and focus handed to the heading first; a saved choice the engine no longer offers falls back to the reel and a `role="status"` line names what was dropped |
| The cell (ADR-045) | a unit of work in the notebook, the six of them read top to bottom and never reordered: a heading row carrying its number in `--text-muted` `--numerals`, its name at 15px, its state and, while it is collapsed, one sentence saying what it holds - the day drawn, how many lines carry an override, which preset. Four cells say one today: 01 the feed, its modes and its stops; 02 the layout's eight characters and when it was made, or "not laid out yet"; 04 the map's theme in the map's own words; 05 what its lines carry. The sentence is derived from the record and never waits on the engine, since a row that was blank while the cell beneath it showed its answer would be the worse of the two. The row is the toggle, inside a real heading so that a notebook of six can be walked by heading and the rail has something to move focus to: a native button with `aria-expanded` over a named group - a group and not a region, because a region is a landmark and six of them would crowd the landmark menu beside the main region and the inspector for no gain. Not a `<details>`, because the state and the summary sit in the row beside the name and `<summary>`'s own layout owns that row. The row's accessible name is its own contents, so what a cell adds to its row it adds to the name. **A cell is exactly one second-level heading.** A panel inside one names itself either with an `h3`, a level below the row, or with an `aria-label` on its region - never with an `h2`, which would be a sibling of the cell's row rather than a child, flattening the outline and taking the cell boundaries out of a heading list, which is the whole of what walking by heading is for (issue 197). Which of the two a panel takes is a judgement about that panel, not a rule about its cell. A heading earns its place where a person has to find the section, step over it, or be told it has arrived: the geographic view's pane is a picture a screen reader cannot read at all, and the diagnostics panel comes and goes on its own, so both are headed. A name without a heading is right where the section is the cell's own subject rather than a part of it - the layout run and the engine's log are named regions inside cell 02 with no heading of their own - and where the row already says what the panel is, as cell 06's export does. Five `h3`s today: Line colours and Line order in 05, In the feed and Where the routes run in 01, What the build had to fudge in 02. Each is `--space-4-6` above, except where the heading opens its cell and so has nothing above to be separated from, and `--space-4-3` below: one rhythm for one level. The value below is what shows where nothing larger follows it - the stage toolbar, the field list - a paragraph's own 1em being larger, and collapsing over it. A collapsed cell keeps its controls in the document rather than unmounting them, since a half-typed value and a running Cancel both live inside one. Which cells are open is where a person is looking, not a setting, and is not stored, as the tab strip's own choice is not; a project's screen opens with 01 to 05 open and 06 closed, which is what the Map tab showed and what the Export tab hid, and 06 is the one that starts closed because the map's own frame is the export's preview only while it is open (A5-01). A cell whose controls wait on the engine says so in one sentence and offers no disabled stand-in |
...
| The rail and its stepper (ADR-045) | the project's own left region, `--space-4-16` collapsed and `--rail-width` (240px) open, fixed under the header on `--surface` with a 1px `--border` on its right and scrolled on its own, as the inspector is on the other side. It takes its space through `--rail-space`, which the main region is padded by and which the map subtracts from `100vw` exactly as it already subtracts the inspector's - a fixed region takes no width from a viewport, and without that term the map would be drawn over it. It holds the six cells as an `<ol>` of buttons - the number, the name and the state as one accessible name - and below them what the project has made. A step says its state the way the cell itself does - the same icon and the same word from one table (`Cell.tsx`), in the state's own colour - because a status carries an icon and a word and is never a hue by itself (principle 1). That it fits was measured rather than assumed: at the rail's own width, against the longest state word, the icon and its gap cost twenty pixels, leave 71 for a name where there were 91, and truncate nothing that was not already truncated - only "Frame and service day", at 137, which is longer than the rail with the icon or without it. The row goes from 37 pixels tall to 40. It earns them: the rail is the only place all six states are seen at once, so which one failed is a glance rather than six words read in turn. The hues follow section 3's scoping table, which is not one rule for all three - `--accent-text` clears every ground, so a running step keeps its colour under the pointer and as the current step, while `--warning` and `--error` are `--surface` and `--surface-raised` only and go back to `--text-muted` on `--surface-hover` and `--surface-selected`, where the icon and the word still carry the state. It is a `<nav>` with a name, not a tablist, because the cells stay in the document and a tablist would promise they do not. `aria-current="step"` follows the scroll without taking focus; choosing a step opens its cell, **scrolls it clear of the header and of the pinned band**, and moves focus to its heading, with no smooth scrolling under reduced motion. The band is half the window deep (A5.5-20), so clearing the header alone would leave the cell behind the map, which looks exactly like a step that did nothing - so the scroll is **computed** against the band's own measured foot at the moment of the press, and the cell's top lands there. Computed and not declared: a `scroll-margin-top` deep enough for the band moves where *every* scroll lands, which is the remedy issue 213 withdrew and the one that reintroduced issue 87's drag. The pressed cell is opened before it is measured, because an open cell is its own room to scroll into; a cell with too little below it to reach the mark lands as high as the document allows. Below 900px it collapses to its numbers, and it is `inert` while the inspector covers the main region |
| Outputs (ADR-045) | what this project has produced, in the rail under the stepper: each finished export by its preset and when it was made, newest first, read from the sidecar the export already writes rather than from memory, so it survives a restart - which is the gap it fills, since today a finished export is findable only in the inspector until the app is closed. Reveal opens the folder and no path is shown (constitution V). A file since moved or deleted reads as gone rather than failing on a press: the row stays, because what was made is still worth saying, and it says so with the warning icon and the sentence on `--surface` - the one status the rail draws, drawn in full. The Reveal that goes with that press is a control that left under a person's finger, so focus is handed to the Outputs heading rather than dropped on the body (A6-07, `focusHandback.ts`). One sentence when there are none |
...
| The pinned preview (ADR-045, A5.5-20) | the viewer's frame, kept on screen while the cells beneath it are read and edited: sticky under the header, its wrapper a child of the notebook's column rather than of any cell, so it is pinned from the top rather than at a scroll threshold - a threshold would need a hysteresis band, a story for a region that changes its relationship to the document mid-scroll, and a test that is flaky by construction, and cells 01 and 02 are better with the map in view. Below 900px it sits above the notebook rather than beside it, which it does at every width, being a child of the column and not a region next to it. **The frame is one element for as long as a project's screen is open.** It is never moved between parents and never remounted - both reload the engine's page, and a reload loses its clock, its view, its labels and its scrub position - so the pinning is CSS on that wrapper and nothing else. Its address changes only deliberately: when a run has rewritten the page file (the address carries how many runs have, since a run writes the same name again and an unchanged address would leave the frame on a document that no longer matches the file) and when cell 06 opens or closes and the export's plan takes the frame. Every one of those is a new document, so the page is asked what it is showing before it goes and told again when it arrives: stopped first, then the view (snapped, never tweened), the labels and the speed, then the clock last of the things that move it, then playing again only if it was. The page is given a deadline to answer in and the frame navigates whether or not it does, because a page the app does not trust must not be able to hold the map on an old document by never replying. What can be given back is what the page will say, and `state()` reports the view, the labels and the clock and not the speed or whether it was playing, so those two are restored only for a caller that set them itself, and the drift of a page that runs on at sixty service-seconds a second is the engine's (issue 29). **Pinned, the map is a band**: the width it already had, and half the window below the header tall. It has to be bounded - a 16:10 map at that width is very nearly the whole window, so pinned it would be the only thing on screen and the cells it is pinned above could not be read, and a sticky box taller than its scrollport sticks by its top edge so its foot could never be scrolled to at all. The bound is on the height and not on the shape: capping the height while keeping the ratio shrinks the width too, and the map would sit small and off-centre on a large screen. The ratio gives way instead, and the page fits itself to whatever box it is given. The band is drawn one step under the header's layer, so the header always draws its own rule, and above the cells, whose own positioned parts come after the map in the document. **What the band hides is not solved** (issue 213): it covers the top of the scrollport, so a control reached with Shift+Tab, or a row something scrolls to before pressing it, can be scrolled to a place behind the map. The obvious remedy makes it worse and was withdrawn - a `scroll-margin-top` on everything in the column as deep as the band, and `scroll-padding-top` on the scrollport, both measured the same: they move where every scroll lands and not only the ones that would have been hidden, and against the colour panel's own geometry they moved the picker's square 139 pixels *up*, to behind the band, so the press that begins a colour drag landed on the map and the picker was dismissed (issue 87's drag). A real remedy is a shallower band, or a scroll container of the notebook's own; the measurements are in `preview.css` beside where the rule would go, and in issue 213, whose acceptance keeps the picker's drag passing |
...
| Export destination (A5.5-19) | where one project's exports go, the last field of cell 06's choices: a 13px label "Where it goes", then either one message sentence saying the exports go to the app's export folder, which Settings names, or - once a folder has been chosen - that folder in the Settings folder row's own block, the monospaced track at 12px, `--text` on `--surface-sunken`, wrapped rather than cut, because a folder's path is the person's own choice and is the one path the app shows; then a toolbar with "Choose folder" carrying the layers icon and, only while there is one to forget, "Use the app's folder"; each is described by whichever of the two lines is showing, so a screen reader says where the file will land; both are unavailable while an export runs, as every other choice is; a refusal - a folder inside the app itself - is a `--error` message under the pair, and the folder that was there stays; "Use the app's folder" removes itself and hands focus to "Choose folder" (A6-07) |
| The engine's log (A5.5-13) | the lines the engine prints for the run that is going, under cell 02's stages, where a run doing something inexplicable can be read beside the stage doing it: a disclosure of its own inside the cell, its row the UI track at 12px reading "Engine log" and, in `--text-muted`, how many lines there are ("no lines yet", "1 line", "212 lines"). It **starts closed** and stays where a person put it for that run, closing again by itself when the next run opens a job of its own, because a panel left open across runs would be showing another run's lines. There is nothing at all until a run has begun. The lines reach the page with every web address's user information, query values and fragment already replaced, in the main process where the engine's `job/log` and `job/progress` notifications are handed to the window (`src/main/engine-ipc.ts`), so nothing on this side redacts anything and the jobs inspector's copy is made from the same redacted bytes. That covers the lines and the sentence beside the progress line, and **not** the engine's error `hint` and `detail`, which still reach the page whole and are drawn on a failure - `withoutPaths` cannot take a key out of a URL, its pattern needing whitespace before the slash it matches - so a failed download still shows a key through the error rather than through the log. Issue #207 is that, across every engine error in the app; until it lands this panel is not a place a secret cannot appear. They are drawn in the job detail's own track - the monospaced face at 12px, `--text-muted` on `--surface-sunken` inside a 1px `--border` - in a box of their own twelve lines of `--line-ui` tall that scrolls, takes focus and carries the focus ring, wrapped rather than cut. The newest line keeps itself in view while the box is resting at its end, by that box's **own `scrollTop` and never `scrollIntoView`**, which scrolls every scrollable ancestor and would take the notebook's column with it out from under the pinned preview on every line; a person who scrolls back through a run is left there until they return to the end. A bounded number of lines is kept - the run's own `MAX_LOG_LINES`, and nothing beyond it on this side - and once any have been dropped one sentence beneath says how many the panel keeps and that the rest are in `engine.log` in the logs folder. "Copy log" is a text button in a toolbar beneath, the inspector's own copy: the page composes the job's text and the main process redacts every web address's user information, query values and fragment and writes the home folder as `~` before the clipboard, and says so in a `role="status"` line. The renderer writes nothing to disk |
| The cell's provenance footer (A5.5-11) | the strip under a cell's controls saying where what the cell holds came from, drawn where `Cell` puts a footer: a `--border` rule above it, `--space-4-2` over `--space-4-4` of padding, `--text-muted` at 12px, and the pairs a definition list wrapped rather than cut - term, colon, value, both halves the strip's own colour, the values in `--numerals`. **Every row is a field of the record or an answer the engine gave**, shortened or formatted; **no row is a conclusion drawn from two of them**, because a conclusion outlives the facts under it - whether a service day is the engine's own choice looks like comparing the day with the window's busiest weekday, and a second layout run moves that weekday while keeping the day, so the strip would credit the engine's day to the person. Three cells have provenance and get one: 02 the stored layout's eight characters, when the engine made it and what it was made with, then **Engine now**, from the supervisor's own state - the tense is in the term, because that one is this moment's while the three above it are the layout's, and a bare "Engine" would say the layout was made by it; the strip asks the engine nothing, so it has its whole height on its first paint and the notebook does not reflow a beat after a run ends (#212), and the LOOM commit stays in Settings and in Copy diagnostics, which is what a bug report is made of; 03 the service day, the days the feed covers, the engine's busiest weekday and the day it was counted from, the last two unconditional and beside the day for a person to compare; 06 the file the last export wrote and the sidecar the engine put beside it. **Cells 01, 04 and 05 get none**: a footer under the line colours saying "changed just now" is chrome pretending to be information, and a cell with nothing true to report draws no strip at all rather than a row of blanks. **The strip carries the facts; the panel above it carries the sentences.** A2-02's moved-inputs sentence and `ServiceDay`'s "the busiest weekday, counted from A, is B" both stay where they are - the first ends in a prompt to act and belongs beside the button that acts, and a strip repeating either would put the same words on screen twice in one cell; a *term* that names the same fact is not that, and stays. The sample page at `?cell-preview` draws the real strip from the real builders, never an imitation of it. No path appears in one, on any platform: a **note** is a sentence and is stripped as the engine's own sentences are, while a **value** is a field and a field rewritten is a field falsified, so a value that would show a path has its row refused instead |
