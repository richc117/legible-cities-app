...
| The cell's provenance footer (A5.5-11) | the strip under a cell's controls saying where what the cell holds came from, drawn where `Cell` puts a footer: a `--border` rule above it, `--space-4-2` over `--space-4-4` of padding, `--text-muted` at 12px, and the pairs a definition list wrapped rather than cut - term, colon, value, both halves the strip's own colour, the values in `--numerals`. Three cells have provenance and get one: 02 the stored layout's eight characters, when the engine made it and what it was made with, then **Engine running** and **LOOM running** - the tense is in the term, because those two are this moment's while the three above them are the layout's, and a bare "Engine" in a strip read top to bottom tells a person the layout was made by it, which the record cannot say; 03 the service day, the days the feed covers, whose choice the day was and, under the engine's own choice, the day it was counted from; 06 the file the last export wrote and the sidecar the engine put beside it. **Cells 01, 04 and 05 get none**: a footer under the line colours saying "changed just now" is chrome pretending to be information, and a cell with nothing true to report draws no strip at all rather than a row of blanks. **The strip carries the facts; the panel above it carries the sentences.** A2-02's moved-inputs sentence and `ServiceDay`'s "the busiest weekday, counted from A, is B" both stay where they are - the first ends in a prompt to act and belongs beside the button that acts, and a strip repeating either would put the same words on screen twice in one cell; a *term* the panel also states is not that, and stays. The sample page at `?cell-preview` draws the real strip from the real builders, never an imitation of it. No path appears in one, on any platform: every string goes out through the same stripping the engine's own sentences get |
