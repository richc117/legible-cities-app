...
| The preview's frame (A5.5-20) | one element, for as long as a project's screen is open. The frame is never moved between parents and never remounted - both reload the engine's page, and a reload loses its clock, its view, its labels and its scrub position - so the pinning is CSS on the wrapper the column already holds and nothing else. Its address changes only deliberately: when a run has rewritten the page file (the address carries how many runs have, since a run writes the same name again and an unchanged address would leave the frame on a document that no longer matches the file) and when cell 06 opens or closes and the export's plan takes the frame. Every one of those is a new document, so the page is asked what it is showing before it goes and told again when it arrives: stopped first, then the view (snapped, never tweened), the labels and the speed, then the clock last of the things that move it, then playing again only if it was. What can be given back is what the page will say, and `state()` reports the view, the labels and the clock and not the speed or whether it was playing, so those two are restored only for a caller that set them itself. Pinned, the map is a band: the width it already had, and half the window below the header tall. It has to be bounded - a 16:10 map at that width is very nearly the whole window, so pinned it would be the only thing on screen and the cells it is pinned above could not be read, and a sticky box taller than its scrollport sticks by its top edge so its foot could never be scrolled to at all. The bound is on the height and not on the shape: capping the height while keeping the ratio shrinks the width too, and the map would sit small and off-centre on a large screen. The ratio gives way instead, and the page fits itself to whatever box it is given |
