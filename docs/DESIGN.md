...
| The cell's provenance footer (A5.5-11) | the strip under a cell's controls saying where what the cell holds came from, drawn where `Cell` puts a footer: a `--border` rule above it, `--space-4-2` over `--space-4-4` of padding, `--text-muted` at 12px, and the pairs a definition list wrapped rather than cut - term, colon, value, both halves the strip's own colour, the values in `--numerals`. Three cells have provenance and get one: 02 the stored layout's eight characters, when the engine made it, what it was made with, and the engine and LOOM this app runs, which are the engine now running and not a claim about the one that made the layout; 03 the service day, the days the feed covers and whose choice the day was; 06 the file the last export wrote and the sidecar the engine put beside it. **Cells 01, 04 and 05 get none**: a footer under the line colours saying "changed just now" is chrome pretending to be information, and a cell with nothing true to report draws no strip at all rather than a row of blanks. Where the record's inputs have moved since the layout was made, the strip says so in A2-02's own sentence, the one the run's panel says above it - one case, one form of words. No path appears in one, on any platform: every string goes out through the same stripping the engine's own sentences get |
