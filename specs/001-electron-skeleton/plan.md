...
| IV. No network, no telemetry | The only fetch the app makes is the main process proxying the Vite dev server on `localhost` in development; a packaged build fetches nothing. CSP on the interface allows `'self'` only for scripts and connections, plus the HMR websocket in development; styles also allow inline since A2-00, for the control kit's shadow styles (`contracts/origin.md`). No update check, no crash reporter, no CDN font. |
