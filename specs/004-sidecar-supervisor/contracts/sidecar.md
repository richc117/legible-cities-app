...
`request`, `cancel`, `inFlight`, `onState`, `onNotification`, `stop`.
...
- `options.deadlineMs` (`RequestOptions`, added by issue 107) gives a request
  ...
  `RangeError` and nothing is sent. On expiry the supervisor sends
...
- A request a bound ended stays counted by `inFlight` until its late answer
  arrives (the client reports the dropped response's id) or the process
  ends, because the engine may still be doing its work; the reset of the
  engine's data refuses while `inFlight` is above zero.
...
1. Set the state to `stopped('quitting')`; reject every request in flight
   with `-32002`; cancel their timers, deadlines included, and forget the
   requests a bound ended.
