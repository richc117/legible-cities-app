...
the main process for the life of the process.
...
- `feeds.add`: `source` must be an `http://` or `https://` URL, or a path
  `pickZip` answered. Any other path is refused: "a feed is added from a
  file chosen in the app, or from a URL".
