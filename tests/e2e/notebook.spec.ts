...
async function withSamplePage(run: (page: Page) => Promise<void>): Promise<void> {
  ...
  writeFileSync(join(engineHome, 'fake-engine.json'), JSON.stringify({ version: PINNED_ENGINE }))
  ...
  try {
    ...
    // renders it in place of the Library (main.tsx).
    await page.evaluate(() => {
      window.location.search = '?cell-preview'
    })
...
/** The first cell of the `ready` section, by the name its row carries. */
...
test('the row says its number, its name and its state as one name', async () => {
  await withSamplePage(async (page) => {
    ...
    // And the summary is part of that name while the cell is collapsed.
    await expect(
      page.getByRole('button', { name: /^06 Export, ready$/ }).first(),
      'a cell with no summary says only its number, name and state',
    ).toBeVisible()
  })
})
